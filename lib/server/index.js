...
Server.storeFactory = function (storeSettings, cwd) {
  ...
  switch (storeSettings.type) {
    case 's3':
      store = new StoreS3(storeSettings.options);
      break;
    default:
      store = new StoreLocal(cwd || serverDefaults.DIRECTORY);  
  }
  return store;
};
...
Server.prototype._createServer = function () {
  ...
  this._client = connect()
    .use(logger(this._debug))
    .use(connect.query())
    .use(connect.compress());
    
  // Custom middleware
  this._middleware.forEach(this._client.use.bind(this._client));
...
function logger (debug) {
  return debug === false ? middlewareNoop : connect.logger();
}

function middlewareNoop (req, res, next) {
  next();
}
